...
func update_tile_size():
	...
	for c in tile_meshes:
		...
		c.mesh.size.z = tile_size
	
func update_shader_material():
	for c in tile_meshes:
		c.material_override = shader_material
		c.set_shader_instance_uniform('seed', seed)
		
func set_shader_param(param_name, param_value):
	for c in tile_meshes:
		c.material_override.set_shader_param(param_name, param_value)
